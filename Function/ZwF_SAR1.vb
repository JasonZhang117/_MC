...
inputs: SAR_high_H( NumericRef );//上抛物趋势线后起点值
...
variables: IntraBarPersist SAR_up_L( Low );//向上突破点Bar的最低价

variables: SAR_low_Q( Low );//下抛物趋势线前起点值
inputs: SAR_low_H( NumericRef );//下抛物趋势线后起点值
...
variables: IntraBarPersist SAR_down_H( High );//向下突破点Bar的最高价
		...
		if condition1 then
			...
			if SAR_low_X > 0 and SAR_high_X > 0 then //向上突破的-上升通道（上下趋势线斜率均为正）
				...
				if SAR_low_X > SAR_high_X then //向上突破的--上升收敛通道（上下趋势线斜率均为正，下趋势线斜率大于上趋势线斜率）
					...
					KcPrice = dKcPrice;
					ZsPrice = dZsPrice;
					Text_New_BN (BarNumber, dKcPrice,"dK----C-11") ; //开仓线
					Text_New_BN (BarNumber, dZsPrice,"dZ----S-11") ; //止损线
					end
				else //上升发散通道（上下趋势线斜率均为正，下趋势线斜率小于趋势线斜率）
					begin
					DKKZ = 12; //多空控制-》向上突破-》上升通道-》发散
					//Text_New_BN (SAR_low_N_H, var0-9,text("DKKZ-->",Round(DKKZ,0))) ;
					dKcPrice = SAR_up_H + 1 point * MinMove; //开仓线-多头
					dZsPrice = SAR_low_H - 1 point * MinMove; //止损线-多头
					KcPrice = dKcPrice;
					ZsPrice = dZsPrice;
					Text_New_BN (BarNumber, dKcPrice,"dK----C-12") ; //开仓线
					Text_New_BN (BarNumber, dZsPrice,"dZ----S-12") ; //止损线
					end;
				end
				...
				if SAR_low_X > SAR_high_X then //下降收敛通道（上下趋势线斜率均为负，下趋势线斜率大于上趋势线斜率）
					...
					KcPrice = dKcPrice;
					ZsPrice = dZsPrice;
					Text_New_BN (BarNumber, dKcPrice,"dK----C-21") ; //开仓线
					Text_New_BN (BarNumber, dZsPrice,"dZ----S-21") ; //止损线
					end
					...
					KcPrice = dKcPrice;
					ZsPrice = dZsPrice;
					Text_New_BN (BarNumber, dKcPrice,"dK----C-22") ; //开仓线
					Text_New_BN (BarNumber, dZsPrice,"dZ----S-22") ; //止损线
					end;
				end
			else
				begin
				if SAR_low_X > 0 and SAR_high_X < 0 then //向上突破的三角通道（上趋势线斜率为负，下趋势线斜率为正）
					...
					KcPrice = dKcPrice;
					ZsPrice = dZsPrice;
					Text_New_BN (BarNumber, dKcPrice,"dK----C-31") ; //开仓线
					Text_New_BN (BarNumber, dZsPrice,"dZ----S-31") ; //止损线
					end
				else
					begin
					DKKZ = 32; //多空控制-》向上突破-》发散形态
					//Text_New_BN (SAR_low_N_H, var0-9,text("DKKZ-->",Round(32,0))) ;
					dKcPrice = SAR_up_H + 1 point * MinMove; //开仓线-多头
					dZsPrice = SAR_low_H - 1 point * MinMove; //止损线-多头
					KcPrice = dKcPrice;
					ZsPrice = dZsPrice;
					Text_New_BN (BarNumber, dKcPrice,"dK----C-32") ; //开仓线
					Text_New_BN (BarNumber, dZsPrice,"dZ----S-32") ; //止损线
					end;
				end;
			end;
		end
		...
		if condition1 then
			...
			if SAR_low_X > 0 and SAR_high_X > 0 then //上升通道（上下趋势线斜率均为正）
				...
				if SAR_low_X > SAR_high_X then //上升收敛通道（上下趋势线斜率均为正，下趋势线斜率大于上趋势线斜率）
					...
					//Text_New_BN (SAR_high_N_H, var0+9,text("DKKZ-->",Round(DKKZ,0))) ;
					kKcPrice = SAR_down_L - 1 point * MinMove;//开仓线-空头
					kZsPrice = SAR_high_H + 1 point * MinMove; //止损线-空头
					KcPrice = kKcPrice;
					ZsPrice = kZsPrice;	
					Text_New_BN (BarNumber, kKcPrice,"kK----C-11") ; //开仓线
					Text_New_BN (BarNumber, kZsPrice,"kZ----S-11") ; //止损线
					end
				else //上升发散通道
					begin
					...
					KcPrice = kKcPrice;
					ZsPrice = kZsPrice;
					Text_New_BN (BarNumber, kKcPrice,"kK----C-12") ; //开仓线
					Text_New_BN (BarNumber, kZsPrice,"kZ----S-12") ; //止损线
					end;
				end
			else if SAR_low_X < 0 and SAR_high_X < 0 then //下降通道（上下趋势线斜率均为负）
				begin
				...
				if SAR_low_X > SAR_high_X then //下降收敛通道（上下趋势线斜率均为负，下趋势线斜率大于上趋势线斜率）
					...
					KcPrice = kKcPrice;
					ZsPrice = kZsPrice;	
					Text_New_BN (BarNumber, kKcPrice,"kK----C-21") ; //开仓线
					Text_New_BN (BarNumber, kZsPrice,"kZ----S-21") ; //止损线					
					end
				else //下降发散通道（上下趋势线斜率均为负，下趋势线斜率小于上趋势线斜率）
					begin
					...
					KcPrice = kKcPrice;
					ZsPrice = kZsPrice;
					Text_New_BN (BarNumber, kKcPrice,"kK----C-22") ; //开仓线
					Text_New_BN (BarNumber, kZsPrice,"kZ----S-22") ; //止损线
					end;
				end
			else
				begin
				if SAR_low_X > 0 and SAR_high_X < 0 then //三角通道（上下趋势线斜率均为负，下趋势线斜率为正）
					...
					KcPrice = kKcPrice;
					ZsPrice = kZsPrice;
					Text_New_BN (BarNumber, kKcPrice,"kK----C-31") ; //开仓线
					Text_New_BN (BarNumber, kZsPrice,"kZ----S-31") ; //止损线
					end
				else
					begin
					DKKZ = -32; //多空控制-》向下突破-》发散形态
					//Text_New_BN (SAR_high_N_H, var0+9,text("DKKZ-->",Round(-32,0))) ;
					kKcPrice = SAR_down_L - 1 point * MinMove;//开仓线-空头
					kZsPrice = SAR_high_H + 1 point * MinMove; //止损线-空头
					KcPrice = kKcPrice;
					ZsPrice = kZsPrice;
					Text_New_BN (BarNumber, kKcPrice,"kK----C-32") ; //开仓线
					Text_New_BN (BarNumber, kZsPrice,"kZ----S-32") ; //止损线
					end;
				end;
			end;
		end;
