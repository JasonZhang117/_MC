...
GVSetNamedInt("G_MP_RSImc",MP); //设置全局变量“PD_MP_m”（持仓方向）
GVSetNamedInt("G_CTT_RSImc",CTT); //设置全局变量“PD_CTT_m”（持仓数量）
...
if RSI_D1_s_D2 crosses above RSI_D_key then //价差(D1-D2)RSI向上突破
	...
	if MP >= 0 then
		...
		ZS_BS_S = ZS_BS; //固定止损
		ZY_BS_S = ZY_BS; //固定止盈
		sellshort("Gk1-S") 1 shares this bar at Close; 
		end;
	end
	...
	if MP <= 0 then
		...
		ZS_BS_S = ZS_BS; //固定止损
		ZY_BS_S = ZY_BS; //固定止盈
		buy("Gk1-B") 1 shares this bar at Close; 
		end;
	end;
...
if BS_B_DK > 0 then
	...
	if MP >= 0 and D1_s_D2 crosses above init_spread then
		...
		ZS_BS_S = ZS_BS; //固定止损
		ZY_BS_S = ZY_BS; //固定止盈
		sellshort("Gk2-S") 1 shares this bar at Close; 
		end;
	end
	...
	if MP <= 0 and D1_s_D2 crosses below init_spread then
		...
		ZS_BS_S = ZS_BS; //固定止损
		ZY_BS_S = ZY_BS; //固定止盈
		buy("Gk2-B") 1 shares this bar at Close; 
		end;
	end;
...
if MP < 0 then
	...
	if D1_s_D2 > ZS_BS_S then //固定止损
		begin
		buytocover ("Gzs1-B") this bar at Close; //止损（价差扩大）
		end
	else if D1_s_D2 < ZY_BS then //移动止盈
		begin
		buytocover ("Gzy1-B") this bar at Close;  //止盈（价差回归）
		end;
	end
else if MP > 0 then
	begin
	if D1_s_D2 < ZS_BS_S then //固定止损
		begin
		sell ("Gzs1-S") this bar at Close; //止损（价差收缩）
		end
	else if D1_s_D2 > ZY_BS then //移动止盈
		begin
		sell ("Gzy1-S") this bar at Close; //止盈（价差回归）
		end;
	end;
