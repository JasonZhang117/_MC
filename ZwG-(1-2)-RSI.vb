...
if RSI_D1_s_D2 crosses above RSI_D_key then
	...
	if MP >= 0 then
		...
		ZS_BS_D = D1_s_D2 + reap_profit_BS;
		ZY_BS_D = D1_s_D2 - reap_profit_BS;
		sellshort("GS1") 1 shares next bar at Open; 
		end;
	end
	...
	if MP <= 0 then
		...
		ZS_BS_K = D1_s_D2 - reap_profit_BS;
		ZY_BS_K = D1_s_D2 + reap_profit_BS;
		buy("GB1") 1 shares next bar at Open; 
		end;
	end;

if MP < 0 then
	begin
	if D1_s_D2 > ZS_BS_D then
		begin
		buytocover ("GD-Z1") next bar at market;
		end
	else if D1_s_D2 < ZY_BS_D then
		begin
		buytocover ("GD-P1") next bar at market;
		end;
	end
else if MP > 0 then
	begin
	if D1_s_D2 < ZS_BS_K then
		begin
		sell ("GK-Z1") next bar at market;
		end
	else if D1_s_D2 > ZY_BS_K then
		begin
		sell ("GK-P1") next bar at market;
		end;
	end;