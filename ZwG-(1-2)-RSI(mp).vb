...
GVSetNamedInt("G_MP_RSImp",MP); //设置全局变量“PD_MP_m”（持仓方向）
GVSetNamedInt("G_CTT_RSImp",CTT); //设置全局变量“PD_CTT_m”（持仓数量）
...
if RSI_D1_s_D2[1] crosses above RSI_D_key then //价差(D1-D2)RSI向上突破
	begin
	init_spread = D1_s_D2;
	ZS_BS = init_spread + reap_profit_BS;
	ZY_BS = init_spread - reap_profit_BS;
	BS_B_DK = 1;
	if MP >= 0 then
		begin
		ZS_BS_S = ZS_BS; //固定止损
