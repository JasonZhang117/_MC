...
GVMP = GVgetNamedInt("G_MP_RSImp",0); //获取主动模型实际持仓方向 
GVCCT = GVgetNamedInt("G_CTT_RSImp",0); //获取主动模型实际持仓数量 
if GVCCT>CTT then //被动模型持仓量大于主动模型持仓量（被动增仓）
	begin
	if GVMP>0 then //主动模型为多头持仓（被动增仓-空头）
		Sell Short("KK") GVCCT-CTT shares next bar at market;
	if GVMP<0 then //主动模型为空头持仓（被动增仓-多头）
